...
namespace PartionnyAccount.Classes.Function.SumInWords
{
    public class Ua
    {
        ...
        public static class RuDateAndMoneyConverter
        {
            ...
            {
                "", "січеня", "лютого", "березеня", "квітеня", "травня", "червня", "липня", "серпня", "вересня", "жовтня", "листопада", "грудня"
            };

            static string zero = "нуль";
            static string firstMale = "одна";
            static string firstFemale = "одна";
            static string firstFemaleAccusative = "одну";
            static string firstMaleGenetive = "одне";
            ...
            public static string CurrencyAddNulls(double _amount, bool _firstCapital)
            {
                //Десять тысяч рублей 67 копеек
                long rublesAmount = (long)Math.Floor(_amount);
                long copecksAmount = ((long)Math.Round(_amount * 100)) % 100;
