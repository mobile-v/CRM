...
{
    public class DirCharTexturesController : ApiController
    {
        #region Classes
        ...
        // GET: api/DirCharTextures
        public async Task<IHttpActionResult> GetDirCharTextures(HttpRequestMessage request)
        {
            try
            {
                ...
                */
                ...
                if (_params.type == "Grid")
                {
                    ...
                    var query =
                        (
                            from dirCharTextures in db.DirCharTextures
                            select new
                            {
                                DirCharTextureID = dirCharTextures.DirCharTextureID,
                                Del = dirCharTextures.Del,
                    ...
                    if (!String.IsNullOrEmpty(_params.parSearch))
                    {
                        ...
                        if (bResult32)
                        {
                            query = query.Where(x => x.DirCharTextureID == iNumber32 || x.DirCharTextureName.Contains(_params.parSearch));
                        }
                        else
                        {
                            query = query.Where(x => x.DirCharTextureName.Contains(_params.parSearch));
                        }
                    }
                    ...
                    query = query.OrderBy(x => x.DirCharTextureName).Skip(_params.Skip).Take(_params.limit);
                    ...
                    //К-во Номенклатуры
                    int dirCount = await Task.Run(() => db.DirCharTextures.Count());
        ...
        // GET: api/DirCharTextures/5
        [ResponseType(typeof(DirCharTexture))]
        ...
        public async Task<IHttpActionResult> PutDirCharTexture(int id, DirCharTexture dirCharTexture)
        {
            ...
            if (id != dirCharTexture.DirCharTextureID) return Ok(returnServer.Return(false, Classes.Language.Sklad.Language.msg8)); //return BadRequest();
        ...
        // DELETE: api/DirCharTextures/5
        [ResponseType(typeof(DirCharTexture))]
        public async Task<IHttpActionResult> DeleteDirCharTexture(int id)
        {
            #region Проверяем Логин и Пароль + Изменяем строку соединения + Права + Разные Функции
            ...
            try
            {
                DirCharTexture dirCharTexture = await db.DirCharTextures.FindAsync(id);
                if (dirCharTexture == null) return Ok(returnServer.Return(false, Classes.Language.Sklad.Language.msg99));


                db.DirCharTextures.Remove(dirCharTexture);
        ...
        private bool DirCharTextureExists(int id)
        {
            return db.DirCharTextures.Count(e => e.DirCharTextureID == id) > 0;
        }
