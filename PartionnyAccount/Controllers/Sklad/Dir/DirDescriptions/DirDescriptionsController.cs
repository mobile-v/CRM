...
namespace PartionnyAccount.Controllers.Sklad.Dir.DirDescriptions
{
    public class DirDescriptionsController : ApiController
    {
        #region Classes
        ...
        // GET: api/DirDescriptions
        public async Task<IHttpActionResult> GetDirDescriptions(HttpRequestMessage request)
        {
            try
            {
                ...
                if (_params.type == "Grid")
                {
                    ...
                    var query =
                        (
                            from dirDescriptions in db.DirDescriptions
                            select new
                            {
                                DirDescriptionID = dirDescriptions.DirDescriptionID,
                                DirDescriptionName = dirDescriptions.DirDescriptionName
                            }
                        );
                    ...
                    query = query.OrderBy(x => x.DirDescriptionName).Skip(_params.Skip).Take(_params.limit);
                    ...
                    //К-во Номенклатуры
                    int dirCount = await Task.Run(() => db.DirDescriptions.Count());
                ...
                else //Tree
                {
                    ...
                    var query =
                        (
                         from x in db.DirDescriptions
                         select new
                         {
                             id = x.DirDescriptionID,
                             text = x.DirDescriptionName,
                             leaf = true
                         }
                        );
        ...
        // GET: api/DirDescriptions/5
        [ResponseType(typeof(DirDescription))]
        public async Task<IHttpActionResult> GetDirDescription(int id, HttpRequestMessage request)
        {
            try
            {
                #region Проверяем Логин и Пароль + Изменяем строку соединения + Права + Разные Функции
                ...
                var query = await Task.Run(() =>
                    (
                        ...
                        select new
                        {
                            ...
                            DirDescriptionName = x.DirDescriptionName
                        }
        ...
        // PUT: api/DirDescriptions/5
        [ResponseType(typeof(void))]
        public async Task<IHttpActionResult> PutDirDescription(int id, DirDescription dirDescription)
        {
            #region Проверяем Логин и Пароль + Изменяем строку соединения + Права + Разные Функции
            ...
            if (id != dirDescription.DirDescriptionID) return Ok(returnServer.Return(false, Classes.Language.Sklad.Language.msg8)); //return BadRequest();
            ...
            try
            {
                db.Entry(dirDescription).State = EntityState.Modified;
        ...
        public async Task<IHttpActionResult> DeleteDirDescription(int id)
        {
            ...
            try
            {
                ...
                if (dirDescription == null) return Ok(returnServer.Return(false, Classes.Language.Sklad.Language.msg99));
        ...
        private bool DirDescriptionExists(int id)
        {
            return db.DirDescriptions.Count(e => e.DirDescriptionID == id) > 0;
        }
