Ext.define("PartionnyAccount.controller.Sklad/Object/List/controllerListObjectPFs", {
    ...
    init: function () {
        this.control({
            ...
            //Грид (itemId=grid)
            'viewListObjectPFs button#btnNew': { click: this.onGrid_BtnNew },
            'viewListObjectPFs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewListObjectPFs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewListObjectPFs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewListObjectPFs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewListObjectPFs button#btnImport': { click: this.onGrid_BtnImport },
            'viewListObjectPFs #DateS': { select: this.onGrid_DateS },
            'viewListObjectPFs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            ...
            'viewListObjectPFs #TriggerSearchGrid': {
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        var Params = [
            ...
            undefined,
            undefined,
            Ext.getCmp("viewListObjectPFs" + aButton.UO_id).GridServerParam1 //(ListObjectID)
        ]
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var ListObjectPFID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.ListObjectPFID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_ListObjectPFs + ListObjectPFID + "/",
                        method: 'DELETE',
    ...
    onGrid_BtnHelp: function (aButton, aEvent, aOptions) {
        window.open(HTTP_Help + "arrival-waybills/", '_blank');
    },
