Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocDomesticExpenses/viewcontrollerDocDomesticExpenses', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocDomesticExpenses',


    //Только для "InterfaceSystem == 3" (layout: 'card')
    //Закрытие и сделать активным другой виджет
    onViewDocDomesticExpensesClose: function (aPanel) {
    ...
    //'viewDocDomesticExpenses button#btnNewCopy': { click: this.Grid_BtnNewCopy },
    ...
    //'viewDocDomesticExpenses button#btnEdit': { click: this.onGrid_BtnEdit },
    ...
    //'viewDocDomesticExpenses button#btnDelete': { click: this.onGrid_BtnDelete },
    ...
    //'viewDocDomesticExpenses button#btnHelp': { click: this.onGrid_BtnHelp },
    onBtnHelpClick: function (aButton, aEvent, aOptions) {
        window.open(HTTP_Help + "retail-retail/", '_blank');
    },

    //'viewDocDomesticExpenses #DateS': { select: this.onGrid_DateS },
    onDateSSelect: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocDomesticExpenses);
    },

    //'viewDocDomesticExpenses #DatePo': { select: this.onGrid_DatePo },
    onDatePoSelect: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocDomesticExpenses);
    },

    // Клик по Гриду
    /*'viewDocDomesticExpenses [itemId=grid]': {
    ...
    /*'viewDocDomesticExpenses #TriggerSearchGrid': {
    ...
    onTriggerClick: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocDomesticExpenses);
    },
    onTriggerSpecialkey: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocDomesticExpenses);
        }
    },
    onTriggerChange: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocDomesticExpenses);
    },
