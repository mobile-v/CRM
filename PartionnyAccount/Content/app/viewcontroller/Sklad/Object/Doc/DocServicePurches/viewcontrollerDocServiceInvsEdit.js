Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocServicePurches/viewcontrollerDocServiceInvsEdit', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocServiceInvsEdit',


    //Только для "InterfaceSystem == 3" (layout: 'card')
    //Закрытие и сделать активным другой виджет
    onViewDocServiceInvsEditClose: function (aPanel) {
    ...
    onGrid_BtnGridRefresh: function (aButton, aEvent, aOptions) {
        ...
        //Выбран Тип
        var DirServiceStatusIDS = parseInt(Ext.getCmp("LoadFrom" + id).getValue()), DirServiceStatusIDPo = parseInt(Ext.getCmp("LoadFrom" + id).getValue());
        ...
        if (Ext.getCmp("LoadFrom" + id).getValue() == null) {
            ...
            if (parseInt(Ext.getCmp("LoadFrom" + id).getValue()) == 0) {
                DirServiceStatusIDS = 1;
                DirServiceStatusIDPo = 9;
            }
        }
        ...
        store.proxy.url =
            ...
            "&DirServiceStatusIDS=" + DirServiceStatusIDS + "&DirServiceStatusIDPo=" + DirServiceStatusIDPo + "" + 
            "&collectionWrapper=DocServiceInvTab";
    ...
    onBtnHeldCancelClick: function (aButton, aEvent, aOptions) {
        Ext.MessageBox.show({
            ...
            fn: function (buttons) { if (buttons == "yes") { viewcontrollerDocServiceInvsEdit_onBtnSaveClick(aButton); } } //, aEvent, aOptions
        });
    },

    //Провести
    onBtnHeldsClick: function (aButton, aEvent, aOptions) {
        viewcontrollerDocServiceInvsEdit_onBtnSaveClick(aButton); //, aEvent, aOptions
    },

    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        viewcontrollerDocServiceInvsEdit_onBtnSaveClick(aButton);
    },
    ...
    onBtnPrintHtmlClick: function (aButton, aEvent, aOptions) {
        ...
        if (Ext.getCmp("DocServiceInvID" + aButton.UO_id).getValue() == null) { Ext.Msg.alert(lanOrgName, txtMsg066); return; }
...
//Функия сохранения
function viewcontrollerDocServiceInvsEdit_onBtnSaveClick(aButton) { //, aEvent, aOptions

    //Спецификация (табличная часть)
    var recordsDocServiceInvTab = [];
    var storeGrid = Ext.getCmp("grid_" + aButton.UO_id).store;
    storeGrid.data.each(function (rec) { recordsDocServiceInvTab.push(rec.data); });
    ...
    var sMethod = "POST";
    var sUrl = HTTP_DocServiceInvs + "?UO_Action=" + aButton.UO_Action;
    if (parseInt(Ext.getCmp("DocServiceInvID" + aButton.UO_id).value) > 0) {
        sMethod = "PUT";
        sUrl = HTTP_DocServiceInvs + "?id=" + parseInt(Ext.getCmp("DocServiceInvID" + aButton.UO_id).value) + "&UO_Action=" + aButton.UO_Action;
    }
    ...
    widgetXForm.submit({
        method: sMethod,
        url: sUrl,
        params: { recordsDocServiceInvTab: Ext.encode(recordsDocServiceInvTab) },
        ...
        success: function (form, action) {
            ...
            if (!Ext.getCmp('DocID' + aButton.UO_id).getValue()) {
                ...
                Ext.getCmp('DocServiceInvID' + aButton.UO_id).setValue(sData.DocServiceInvID);
                Ext.getCmp('NumberInt' + aButton.UO_id).setValue(sData.DocServiceInvID);
                Ext.Msg.alert(lanOrgName, lanDataSaved + "<br />" + txtMsg096 + sData.DocServiceInvID);

                Ext.getCmp('viewDocServiceInvsEdit' + aButton.UO_id).setTitle(Ext.getCmp('viewDocServiceInvsEdit' + aButton.UO_id).title + " (" + Ext.getCmp("DocServiceInvID" + aButton.UO_id).getValue() + ")");
            ...
            if (aButton.UO_Action == "save_close" || aButton.UO_Action == "held") { Ext.getCmp(aButton.UO_idMain).close(); }
                ...
                locStore.on('load', function () {
                    controllerDocServiceInvsEdit_RecalculationSums(Ext.getCmp(aButton.UO_idMain).UO_id, false);
                });*/
            }
