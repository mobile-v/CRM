Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocSecondHandPurches/viewcontrollerDocSecondHandRetailsEdit', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocSecondHandRetailsEdit',


    //Только для "InterfaceSystem == 3" (layout: 'card')
    //Закрытие и сделать активным другой виджет
    onViewDocSecondHandRetailsEditClose: function (aPanel) {
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        viewcontrollerDocSecondHandRetailsEdit_onTriggerSearchGridClick(aButton.UO_id, HTTP_Rem2Parties);
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            viewcontrollerDocSecondHandRetailsEdit_onTriggerSearchGridClick(f.UO_id, HTTP_Rem2Parties);
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        
        if (textReal.length > 2) {
            viewcontrollerDocSecondHandRetailsEdit_onTriggerSearchGridClick(e.UO_id, HTTP_Rem2Parties);
        }
    },
    ...
    onGrid_BtnGridAddPosition: function (aButton, aEvent, aOptions) {
        ...
        */
        ...
        ObjectEditConfig("viewDocSecondHandRetailTabsEdit", Params);
    ...
    onGrid_BtnGridEdit: function (aButton, aEvent, aOptions) {
        ...
        if (ListObjectID == 56) {
            ...
            ObjectEditConfig("viewDocSecondHandRetailReturnTabsEdit", Params);
        }
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    if (ListObjectID == 56) {
                        ...
                        //var DocSecondHandRetailID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.NumberReal; // == DocSecondHandRetailID;
                        var DocSecondHandRetailID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocSecondHandRetailID;
                        //Запрос на удаление
                        Ext.Ajax.request({
                            timeout: varTimeOutDefault,
                            url: HTTP_DocSecondHandRetails + DocSecondHandRetailID + "/",
                            method: 'DELETE',
                        ...
                        //Выбранный ID-шник Грида
                        //var DocSecondHandRetailReturnID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.NumberReal; // == DocSecondHandRetailReturnID;
                        var DocSecondHandRetailReturnID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocSecondHandRetailReturnID;
                        //Запрос на удаление
                        Ext.Ajax.request({
                            timeout: varTimeOutDefault,
                            url: HTTP_DocSecondHandRetailReturns + DocSecondHandRetailReturnID + "/",
                            method: 'DELETE',
                            success: function (result) {
                                loadingMask.hide();
                                var sData = Ext.decode(result.responseText);
    ...
    onGrid_BtnGridRefresh: function (aButton, aEvent, aOptions) {
        ...
        store.on('load', function () {
            ...
            //Обновляем партиис
            var storeRem2PartiesGrid = Ext.getCmp("gridParty_" + aButton.UO_id).store;
    ...
    onGrid_BtnGridActWriteOff: function (aButton, aEvent, aOptions) {
        ...
        */
        ...
        ObjectEditConfig("viewDocSecondHandRetailActWriteOffsEdit", Params);
            ...
            Ext.Msg.confirm("Confirmation", "Печатать чек на ККМ?", function (btnText) {
                if (btnText === "no") {
                    ...
                    if (ListObjectID == 66) {
                        ...
                        RegisterCheck(aButton, FormData, viewcontrollerDocSecondHandRetailsEdit_Kkm, 0, false);
                        ...
                        RegisterCheck(aButton, FormData, viewcontrollerDocSecondHandRetailsEdit_Kkm, 1, false);
...
function viewcontrollerDocSecondHandRetailsEdit_RecalculationSums111(id, ShowMsg) {
...
//Поиск
function viewcontrollerDocSecondHandRetailsEdit_onTriggerSearchGridClick(UO_id, url) {
...
function viewcontrollerDocSecondHandRetailsEdit_Kkm(Rezult, textStatus, jqXHR, aButton) {
    ...
    if (Rezult.Status == 0) {
        ...
        Ext.Ajax.request({
            timeout: varTimeOutDefault,
            waitMsg: lanUpload,
            url: HTTP_DocSecondHandRetails + IdcallModelData[0].data.DocID + "/0/0/?KKMSCheckNumber=" + Rezult.CheckNumber + "&KKMSIdCommand=" + Rezult.IdCommand,
            method: 'PUT',
            success: function (result) {
                var sData = Ext.decode(result.responseText);
