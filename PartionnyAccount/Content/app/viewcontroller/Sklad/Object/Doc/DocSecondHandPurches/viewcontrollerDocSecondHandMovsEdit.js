Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocSecondHandPurches/viewcontrollerDocSecondHandMovsEdit', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocSecondHandMovsEdit',


    //Только для "InterfaceSystem == 3" (layout: 'card')
    //Закрытие и сделать активным другой виджет
    onViewDocSecondHandMovsEditClose: function (aPanel) {
    ...
    onGrid_BtnGridRefresh: function (aButton, aEvent, aOptions) {
        ...
        //Выбран Тип
        var DirSecondHandStatusIDS = parseInt(Ext.getCmp("LoadFrom" + id).getValue()), DirSecondHandStatusIDPo = parseInt(Ext.getCmp("LoadFrom" + id).getValue());
        if (Ext.getCmp("LoadFrom" + id).getValue() == null) {
            Ext.Msg.alert(lanOrgName, "Выбирите тип загружаемых аппаратов!");
            return;
        }
        else {
            if (parseInt(Ext.getCmp("LoadFrom" + id).getValue()) == 0) {
        ...
        var store = Ext.getCmp("gridParty_" + id).store;
        store.proxy.url =
            HTTP_DocSecondHandPurches +
            "?DirContractorIDOrg=" + Ext.getCmp("DirContractorIDOrg" + id).getValue() +
            "&DirWarehouseID=" + Ext.getCmp("DirWarehouseIDFrom" + id).getValue() +
        ...
        store.on('load', function () {
            ...
            Ext.getCmp("LoadFrom" + id).setValue(Ext.getCmp("LoadFrom" + id).getValue());
        });
    ...
    onGrid_BtnGridAddPosition: function (aButton, aEvent, aOptions) {
        ...
        */
        ...
        ObjectEditConfig("viewDocSecondHandMovTabsEdit", Params);
    },
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    if (selection) {
                        ...
                        controllerDocSecondHandMovsEdit_RecalculationSums(aButton.UO_id, false)
                    }
                }
            }
        });
    ...
    onBtnHeldCancelClick: function (aButton, aEvent, aOptions) {
        Ext.MessageBox.show({
            ...
            fn: function (buttons) { if (buttons == "yes") { controllerDocSecondHandMovsEdit_onBtnSaveClick(aButton, aEvent, aOptions); } }
        });
    },

    //Провести
    onBtnHeldsClick: function (aButton, aEvent, aOptions) {
        controllerDocSecondHandMovsEdit_onBtnSaveClick(aButton, aEvent, aOptions);
    },
    ...
    onBtnPrintHtmlClick: function (aButton, aEvent, aOptions) {
        ...
        if (Ext.getCmp("DocSecondHandMovID" + aButton.UO_id).getValue() == null) { Ext.Msg.alert(lanOrgName, txtMsg066); return; }
...
//Функия сохранения
function controllerDocSecondHandMovsEdit_onBtnSaveClick(aButton, aEvent, aOptions) {
    ...
    if (Ext.getCmp("viewDocSecondHandMovsEdit" + id).UO_GridSave && (Ext.getCmp("DirEmployeeIDCourier" + id).getValue() == "" || Ext.getCmp("DirEmployeeIDCourier" + id).getValue() == null)) {
    ...
    if (!Ext.getCmp("DirContractorIDOrg" + id).getValue()) {
        Ext.Msg.alert(lanOrgName, "Выберите Организацию!");
        return;
    }
    if (!Ext.getCmp("LoadFrom" + id).getValue() && Ext.getCmp("LoadFrom" + id).getValue() !=0) {
        debugger;
        Ext.Msg.alert(lanOrgName, "Выберите LoadFrom!");
        return;
    }
    ...
    var recordsDocSecondHandMovTab = [];
    var storeGrid = Ext.getCmp("grid_" + id).store;
    storeGrid.data.each(function (rec) { recordsDocSecondHandMovTab.push(rec.data); });
    ...
    var sMethod = "POST";
    var sUrl = HTTP_DocSecondHandMovs + "?UO_Action=" + aButton.UO_Action;
    if (parseInt(Ext.getCmp("DocSecondHandMovID" + id).value) > 0) {
        sMethod = "PUT";
        sUrl = HTTP_DocSecondHandMovs + "?id=" + parseInt(Ext.getCmp("DocSecondHandMovID" + id).value) + "&UO_Action=" + aButton.UO_Action;
    }
    ...
    widgetXForm.submit({
        ...
        params: { recordsDocSecondHandMovTab: Ext.encode(recordsDocSecondHandMovTab) },
        ...
        success: function (form, action) {
            ...
            if (!Ext.getCmp('DocID' + id).getValue()) {
                ...
                Ext.getCmp('DocSecondHandMovID' + id).setValue(sData.DocSecondHandMovID);
                Ext.getCmp('NumberInt' + aButton.UO_id).setValue(sData.DocSecondHandMovID);
                Ext.Msg.alert(lanOrgName, lanDataSaved + "<br />" + txtMsg096 + sData.DocSecondHandMovID + "<br />" + sMsg);

                Ext.getCmp('viewDocSecondHandMovsEdit' + aButton.UO_id).setTitle(Ext.getCmp('viewDocSecondHandMovsEdit' + aButton.UO_id).title + " (" + Ext.getCmp("DocSecondHandMovID" + aButton.UO_id).getValue() + ")");
...
function viewcontrollerDocSecondHandMovsEdit_RecalculationSums(id, ShowMsg) {
