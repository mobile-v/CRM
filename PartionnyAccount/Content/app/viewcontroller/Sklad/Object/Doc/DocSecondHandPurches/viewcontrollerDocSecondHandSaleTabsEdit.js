Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocSecondHandPurches/viewcontrollerDocSecondHandSaleTabsEdit', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocSecondHandSaleTabsEdit',



    onChangeDiscount: function (textfield, valueNew, valueOld, e) {
        viewcontrollerDocSecondHandSaleTabsEdit_RecalculationSums(textfield.UO_id);
    },
                ...
                Ext.MessageBox.show({
                    ...
                    fn: function (btn) {
                        if (btn == "yes") {
                            ...
                            viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick_Union(aButton);
                        }
                        else if (btn == "no") {
    ...
    onBtnHeldCancelClick: function (aButton, aEvent, aOptions) {
        Ext.MessageBox.show({
            ...
            fn: function (buttons) { if (buttons == "yes") { viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick(aButton, aEvent, aOptions); } }
        });
    },
...
function viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick_Union(aButton) {
    ...
    if (varKKMSActive) {
        Ext.Msg.confirm("Confirmation", "Печатать чек на ККМ?", function (btnText) {
            if (btnText === "no") {
                ...
                viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick(aButton);
                ...
                RegisterCheck(aButton, FormData, viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick_Kkm, 0, false);
            }
        }, this);
    }
    else {
...
function viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick_Kkm(Rezult, textStatus, jqXHR, aButton) {
...
//Функия сохранения
function viewcontrollerDocSecondHandSaleTabsEdit_onBtnSaveClick(aButton, aEvent, aOptions) {
    ...
    /*
    var recordsDocSecondHandSaleTab = [];
    var storeGrid = Ext.getCmp("grid_" + aButton.UO_id).store;
    storeGrid.data.each(function (rec) { recordsDocSecondHandSaleTab.push(rec.data); });
    */
    ...
    var sMethod = "POST";
    var sUrl = HTTP_DocSecondHandSales + "?UO_Action=" + aButton.UO_Action;
    if (parseInt(Ext.getCmp("DocSecondHandSaleID" + aButton.UO_id).value) > 0) {
        sMethod = "PUT";
        sUrl = HTTP_DocSecondHandSales + "?id=" + parseInt(Ext.getCmp("DocSecondHandSaleID" + aButton.UO_id).value) + "&UO_Action=" + aButton.UO_Action;
    }
    ...
    widgetXForm.submit({
        method: sMethod,
        url: sUrl,
        //params: { recordsDocSecondHandSaleTab: Ext.encode(recordsDocSecondHandSaleTab) },
        ...
        success: function (form, action) {
            ...
            if (!Ext.getCmp('DocID' + aButton.UO_id).getValue()) {
                ...
                Ext.getCmp('DocSecondHandSaleID' + aButton.UO_id).setValue(sData.DocSecondHandSaleID);
            }
            ...
            if (Ext.getCmp(aButton.UO_idCall) != undefined && Ext.getCmp(aButton.UO_idCall).store != undefined) {
                ...
                storeGrid.on('load', function () {
                    //viewcontrollerDocSecondHandSalesEdit_RecalculationSums(Ext.getCmp(aButton.UO_idCall).UO_id, false)
...
function viewcontrollerDocSecondHandSaleTabsEdit_RecalculationSums(id) {
