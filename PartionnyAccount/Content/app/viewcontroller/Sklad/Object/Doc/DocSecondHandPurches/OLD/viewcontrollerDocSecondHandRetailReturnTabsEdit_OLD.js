Ext.define('PartionnyAccount.viewcontroller.Sklad/Object/Doc/DocSecondHandPurches/viewcontrollerDocSecondHandRetailReturnTabsEdit', {
    extend: 'Ext.app.ViewController',

    alias: 'controller.viewcontrollerDocSecondHandRetailReturnTabsEdit',



    onChangeDiscount: function (textfield, valueNew, valueOld, e) {
        viewcontrollerDocSecondHandRetailReturnTabsEdit_RecalculationSums(textfield.UO_id);
    },
                ...
                Ext.MessageBox.show({
                    ...
                    fn: function (btn) {
                        if (btn == "yes") {
                            ...
                            viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick_Union(aButton);
                        }
                        else if (btn == "no") {
    ...
    onBtnHeldCancelClick: function (aButton, aEvent, aOptions) {
        Ext.MessageBox.show({
            ...
            fn: function (buttons) { if (buttons == "yes") { viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick_Union(aButton); } }
        });
    },
...
function viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick_Union(aButton) {
    ...
    if (varKKMSActive) {
        Ext.Msg.confirm("Confirmation", "Печатать чек на ККМ?", function (btnText) {
            if (btnText === "no") {
                ...
                viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick(aButton);
            }
            else if (btnText === "yes") {
                ...
                RegisterCheck(aButton, FormData, viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick_Kkm, 1, false);
            }
        }, this);
    }
    else {
...
function viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick_Kkm(Rezult, textStatus, jqXHR, aButton) {
...
//Функия сохранения
function viewcontrollerDocSecondHandRetailReturnTabsEdit_onBtnSaveClick(aButton, aEvent, aOptions) {
    ...
    var recordsDocSecondHandRetailReturnTab = [];
    var storeGrid = Ext.getCmp("grid_" + aButton.UO_id).store;
    storeGrid.data.each(function (rec) { recordsDocSecondHandRetailReturnTab.push(rec.data); });
    ...
    var sMethod = "POST";
    var sUrl = HTTP_DocSecondHandRetailReturns + "?UO_Action=" + aButton.UO_Action;
    if (parseInt(Ext.getCmp("DocSecondHandRetailReturnID" + aButton.UO_id).value) > 0) {
        sMethod = "PUT";
        sUrl = HTTP_DocSecondHandRetailReturns + "?id=" + parseInt(Ext.getCmp("DocSecondHandRetailReturnID" + aButton.UO_id).value) + "&UO_Action=" + aButton.UO_Action;
    }
    ...
    widgetXForm.submit({
        method: sMethod,
        url: sUrl,
        params: { recordsDocSecondHandRetailReturnTab: Ext.encode(recordsDocSecondHandRetailReturnTab) },
        ...
        success: function (form, action) {
            ...
            if (Ext.getCmp(aButton.UO_idCall) != undefined && Ext.getCmp(aButton.UO_idCall).store != undefined) {
                ...
                storeGrid.on('load', function () {
                    //viewcontrollerDocSecondHandRetailReturnsEdit_RecalculationSums(Ext.getCmp(aButton.UO_idCall).UO_id, false)
...
function viewcontrollerDocSecondHandRetailReturnTabsEdit_RecalculationSums(id) {
