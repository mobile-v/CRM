Ext.define("PartionnyAccount.view.Sklad/Object/Doc/DocSecondHandPurches/viewDocSecondHandInventoriesEdit", {

    extend: InterfaceSystemObjName,
    alias: "widget.viewDocSecondHandInventoriesEdit",
    ...
    //Контроллер
    controller: 'viewcontrollerDocSecondHandInventoriesEdit',
    listeners: { close: 'onViewDocSecondHandInventoriesEditClose' },
    ...
    initComponent: function () {
        ...
        var PanelDocumentDetails = Ext.create('Ext.panel.Panel', {
            ...
            items: [
                ...
                { xtype: 'textfield', fieldLabel: "DocSecondHandInventoryID", name: "DocSecondHandInventoryID", id: "DocSecondHandInventoryID" + this.UO_id, readOnly: true, allowBlank: true, hidden: true },
...

                {
                    ...
                    hidden: true,
                    items: [
                        { xtype: 'textfield', fieldLabel: "№", name: "DocSecondHandInventoryID", id: "DocSecondHandInventoryID" + this.UO_id, readOnly: true, flex: 1, allowBlank: true },
        ...
        var PanelGrid = Ext.create('Ext.grid.Panel', {
            id: "grid_" + this.UO_id, UO_id: this.UO_id, UO_idMain: this.UO_idMain, UO_idCall: this.UO_idCall, UO_View: "viewDocSecondHandInventoriesEdit",
            ...
            columns: [
                ...
                //Документ БУ
                { text: "Документ", dataIndex: "DocSecondHandPurchID", width: 100, style: "height: 25px;", tdCls: 'x-change-cell' },
                ...
                {
                    text: "Статус", dataIndex: "ExistName", width: 140, tdCls: 'x-change-cell102',
                    editor: {
                        xtype: 'combo',
                        name: "ExistName",
                        ...
                        listeners: {
                            ...
                            select: function (comboBox, records) {
                                ...
                                comboBox.inputEl.setStyle({
                                    'background-image': 'url(' + records.get('img') + ')',
                            ...
                            */
                            ...
                            blur: function (field, event, eOpts) {
                                ...
                                for (var i = 0; i < field.UO_store.model.fields.length; i++) {
                                    if (field.UO_store.data.items[i].data.ExistName == "Списывается с ЗП") {
                                        SumOfVATCurrency1 += field.UO_store.data.items[i].data.Quantity * field.UO_store.data.items[i].data.PriceCurrency;
                                    }
                                    else if (field.UO_store.data.items[i].data.ExistName == "На разбор") {
                                        SumOfVATCurrency2 += field.UO_store.data.items[i].data.Quantity * field.UO_store.data.items[i].data.PriceCurrency;
                                    }
                                }

                                Ext.getCmp("SumOfVATCurrency1" + field.UO_id).setValue(SumOfVATCurrency1);
                                Ext.getCmp("SumOfVATCurrency2" + field.UO_id).setValue(SumOfVATCurrency2);
                                */
                            },

                            /*
                            lastselectedchanged: function (field, event, eOpts) {
                                var SumOfVATCurrency1 = 0, SumOfVATCurrency2 = 0;
                                for (var i = 0; i < field.UO_store.model.fields.length; i++) {
                                    if (field.UO_store.data.items[i].data.ExistName == "Списывается с ЗП") {
                                        SumOfVATCurrency1 += field.UO_store.data.items[i].data.Quantity * field.UO_store.data.items[i].data.PriceCurrency;
                    ...
                    if (record.store.UO_viewConfig) {
                        ...
                        for (var i = 0; i < record.store.model.fields.length; i++) {
                            if (record.store.data.items[i].data.ExistName == "Списывается с ЗП") {
                                SumOfVATCurrency1 += record.store.data.items[i].data.Quantity * record.store.data.items[i].data.PriceCurrency;
                            }
                            else if (record.store.data.items[i].data.ExistName == "На разбор") {
                                SumOfVATCurrency2 += record.store.data.items[i].data.Quantity * record.store.data.items[i].data.PriceCurrency;
                            }
                        }
            ...
            items: [
                {
                    ...
                    items: [
                        //Суммы
                        { xtype: 'textfield', fieldLabel: "Списать с ЗП", name: "SumOfVATCurrency1", id: "SumOfVATCurrency1" + this.UO_id, regex: /^[+\-]?\d+(?:\.\d+)?$/, width: 250, readOnly: true, allowBlank: true },
                        { xtype: 'textfield', fieldLabel: "Разбор", name: "SumOfVATCurrency2", id: "SumOfVATCurrency2" + this.UO_id, regex: /^[+\-]?\d+(?:\.\d+)?$/, width: 250, readOnly: true, allowBlank: true, margin: "0 0 0 15" },
                    ]
                },
