Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirCashOffices/controllerDirCashOffices", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCashOffices': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCashOffices [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCashOffices [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCashOffices [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCashOffices [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCashOffices [itemId=tree]': {
            ...
            //Валюта
            'viewDirCashOffices button#btnDirCurrencyEdit': { click: this.onBtnDirCurrencyEditClick },
            'viewDirCashOffices button#btnDirCurrencyReload': { click: this.onBtnDirCurrencyReloadClick },
             
            
            // === Кнопки: Сохранение, Отмена и Помощь === === ===
            'viewDirCashOffices button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCashOffices button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCashOffices button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCashOffices_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCashOffices_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCashOffices_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCashOffices + rec.get('id') + "/",
    ...
    onBtnDirCurrencyEditClick: function (aButton, aEvent, aOptions) {
        var Params = [
            aButton.id,
            false, //UO_Center
            false, //UO_Modal
    ...
    onBtnDirCurrencyReloadClick: function (aButton, aEvent, aOptions) {
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCashOffices;
        if (parseInt(Ext.getCmp("DirCashOfficeID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCashOffices + "?id=" + parseInt(Ext.getCmp("DirCashOfficeID" + aButton.UO_id).value);
        }
...
function controllerDirCashOffices_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCashOffices_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCashOffices_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCashOfficeID" + id).setValue(0);
                Ext.getCmp("DirCashOfficeName" + id).setValue(Ext.getCmp("DirCashOfficeName" + id).getValue() + " (копия)");
...
function controllerDirCashOffices_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCashOfficeID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCashOfficeID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCashOffices + DirCashOfficeID + "/",
                    method: 'DELETE',
...
function controllerDirCashOffices_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCashOffices + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
