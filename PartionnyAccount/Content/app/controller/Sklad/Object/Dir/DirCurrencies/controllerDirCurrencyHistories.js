Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirCurrencies/controllerDirCurrencyHistories", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCurrencyHistories': { close: this.this_close },

            //Грид (itemId=grid)
            'viewDirCurrencyHistories button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDirCurrencyHistories #TriggerSearchGrid': {
            ...
            // Клик по Гриду
            'viewDirCurrencyHistories [itemId=grid]': { selectionchange: this.onGrid_selectionchange },
        });
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DirCurrencyHistoryID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DirCurrencyHistoryID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DirCurrencyHistories + DirCurrencyHistoryID + "/",
                        method: 'DELETE',
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDir(aButton, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DirCurrencyHistories);
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDir(f, f.value, HTTP_DirCurrencyHistories);
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDir(e, textReal, HTTP_DirCurrencyHistories);
    },
...
//Поиск в Grid-e
/*
function controllerDirCurrencyHistories_Grid_Searc(UO_id, txt) {
    //Получаем storeGrid и делаем load()
    var storeGrid = Ext.getCmp("grid_" + UO_id).getStore();
    storeGrid.proxy.url = HTTP_DirCurrencyHistories + "?parSearch=" + txt;
    storeGrid.load();
}
*/