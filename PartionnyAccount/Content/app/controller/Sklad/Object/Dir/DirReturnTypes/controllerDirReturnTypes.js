Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirReturnTypes/controllerDirReturnTypes", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirReturnTypes': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirReturnTypes [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirReturnTypes [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirReturnTypes [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirReturnTypes [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirReturnTypes [itemId=tree]': {
            ...
            'viewDirReturnTypes button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirReturnTypes button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirReturnTypes button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirReturnTypes_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirReturnTypes_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirReturnTypes_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirReturnTypes + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirReturnTypes;
        if (parseInt(Ext.getCmp("DirReturnTypeID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirReturnTypes + "?id=" + parseInt(Ext.getCmp("DirReturnTypeID" + aButton.UO_id).value);
        }
...
function controllerDirReturnTypes_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirReturnTypes_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirReturnTypes_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirReturnTypeID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirReturnTypeID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirReturnTypes + DirReturnTypeID + "/",
                    method: 'DELETE',
...
function controllerDirReturnTypes_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirReturnTypes + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
