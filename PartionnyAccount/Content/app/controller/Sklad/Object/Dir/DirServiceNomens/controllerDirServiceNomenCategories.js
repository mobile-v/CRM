Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirServiceNomens/controllerDirServiceNomenCategories", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirServiceNomenCategories': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirServiceNomenCategories [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirServiceNomenCategories [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirServiceNomenCategories [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirServiceNomenCategories [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirServiceNomenCategories [itemId=tree]': {
            ...
            'viewDirServiceNomenCategories button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirServiceNomenCategories button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirServiceNomenCategories button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirServiceNomenCategories_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirServiceNomenCategories_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirServiceNomenCategories_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        if (Ext.getCmp(view.grid.UO_idMain).UO_Function_Tree == undefined || !varSelectOneClick) {
            ...
            widgetXForm.load({
                ...
                waitMsg: lanLoading,
                url: HTTP_DirServiceNomenCategories + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirServiceNomenCategories;
        if (parseInt(Ext.getCmp("DirServiceNomenCategoryID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirServiceNomenCategories + "?id=" + parseInt(Ext.getCmp("DirServiceNomenCategoryID" + aButton.UO_id).value);
        }
...
function controllerDirServiceNomenCategories_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirServiceNomenCategories_onTree_folderNewSub(id) {
    /*
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirServiceNomenCategories_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirServiceNomenCategoryID" + id).setValue(0);
                Ext.getCmp("DirServiceNomenCategoryName" + id).setValue(Ext.getCmp("DirServiceNomenCategoryName" + id).getValue() + " (копия)");
...
function controllerDirServiceNomenCategories_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirServiceNomenCategoryID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirServiceNomenCategoryID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceNomenCategories + DirServiceNomenCategoryID + "/",
                    method: 'DELETE',
...
function controllerDirServiceNomenCategories_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceNomenCategories + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
