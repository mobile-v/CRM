Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirServiceNomens/controllerDirServiceNomenTypicalFaults", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirServiceNomenTypicalFaults': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirServiceNomenTypicalFaults [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirServiceNomenTypicalFaults [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirServiceNomenTypicalFaults [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirServiceNomenTypicalFaults [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirServiceNomenTypicalFaults [itemId=tree]': {
            ...
            'viewDirServiceNomenTypicalFaults button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirServiceNomenTypicalFaults button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirServiceNomenTypicalFaults button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirServiceNomenTypicalFaults_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirServiceNomenTypicalFaults_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirServiceNomenTypicalFaults_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        if (Ext.getCmp(view.grid.UO_idMain).UO_Function_Tree == undefined || !varSelectOneClick) {
            ...
            widgetXForm.load({
                ...
                waitMsg: lanLoading,
                url: HTTP_DirServiceNomenTypicalFaults + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirServiceNomenTypicalFaults;
        if (parseInt(Ext.getCmp("DirServiceNomenTypicalFaultID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirServiceNomenTypicalFaults + "?id=" + parseInt(Ext.getCmp("DirServiceNomenTypicalFaultID" + aButton.UO_id).value);
        }
...
function controllerDirServiceNomenTypicalFaults_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirServiceNomenTypicalFaults_onTree_folderNewSub(id) {
    /*
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirServiceNomenTypicalFaults_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirServiceNomenTypicalFaultID" + id).setValue(0);
                Ext.getCmp("DirServiceNomenTypicalFaultName" + id).setValue(Ext.getCmp("DirServiceNomenTypicalFaultName" + id).getValue() + " (копия)");
...
function controllerDirServiceNomenTypicalFaults_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirServiceNomenTypicalFaultID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirServiceNomenTypicalFaultID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceNomenTypicalFaults + DirServiceNomenTypicalFaultID + "/",
                    method: 'DELETE',
...
function controllerDirServiceNomenTypicalFaults_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceNomenTypicalFaults + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
