Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirBanks/controllerDirBanksGrid", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirBanksGrid': { close: this.this_close },

            //Грид (itemId=grid)
            'viewDirBanksGrid button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDirBanksGrid button#btnHelp': { click: this.onGrid_BtnHelp },
            // Клик по Гриду
            'viewDirBanksGrid [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDirBanksGrid #TriggerSearchGrid': {
    ...
    onGrid_BtnEdit: function (aButton, aEvent, aOptions) {
        if (Ext.getCmp(aButton.UO_idMain).UO_Function_Grid == undefined) {
            ...
            ObjectEditConfig("viewDocBankSumsEdit", Params);
        }
        else {
