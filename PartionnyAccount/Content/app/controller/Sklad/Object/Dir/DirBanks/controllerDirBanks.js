Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirBanks/controllerDirBanks", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirBanks': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirBanks [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirBanks [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirBanks [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirBanks [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirBanks [itemId=tree]': {
            ...
            //Валюта
            'viewDirBanks button#btnDirCurrencyEdit': { click: this.onBtnDirCurrencyEditClick },
            'viewDirBanks button#btnDirCurrencyReload': { click: this.onBtnDirCurrencyReloadClick },
             
            
            // === Кнопки: Сохранение, Отмена и Помощь === === ===
            'viewDirBanks button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirBanks button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirBanks button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirBanks_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirBanks_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirBanks_onTree_folderDel(aButton.UO_id);
    },
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirBanks;
        if (parseInt(Ext.getCmp("DirBankID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirBanks + "?id=" + parseInt(Ext.getCmp("DirBankID" + aButton.UO_id).value);
        }
    ...
    onBtnHelpClick: function (aButton, aEvent, aOptions) {
        window.open(HTTP_Help + "spravochnik-bank/", '_blank');
    }
});
...
function controllerDirBanks_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirBanks_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirBanks_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirBankID" + id).setValue(0);
                Ext.getCmp("DirBankName" + id).setValue(Ext.getCmp("DirBankName" + id).getValue() + " (копия)");
...
function controllerDirBanks_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirBankID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirBankID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirBanks + DirBankID + "/",
                    method: 'DELETE',
...
function controllerDirBanks_onTree_folderSubNull(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirBanks + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
