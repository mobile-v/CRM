Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirServiceDiagnosticRresults/controllerDirServiceDiagnosticRresults", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirServiceDiagnosticRresults': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirServiceDiagnosticRresults [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirServiceDiagnosticRresults [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirServiceDiagnosticRresults [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirServiceDiagnosticRresults [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirServiceDiagnosticRresults [itemId=tree]': {
            ...
            'viewDirServiceDiagnosticRresults button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirServiceDiagnosticRresults button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirServiceDiagnosticRresults button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirServiceDiagnosticRresults_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirServiceDiagnosticRresults_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirServiceDiagnosticRresults_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirServiceDiagnosticRresults + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirServiceDiagnosticRresults;
        if (parseInt(Ext.getCmp("DirServiceDiagnosticRresultID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirServiceDiagnosticRresults + "?id=" + parseInt(Ext.getCmp("DirServiceDiagnosticRresultID" + aButton.UO_id).value);
        }
...
function controllerDirServiceDiagnosticRresults_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirServiceDiagnosticRresults_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirServiceDiagnosticRresults_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirServiceDiagnosticRresultID" + id).setValue(0);
                Ext.getCmp("DirServiceDiagnosticRresultName" + id).setValue(Ext.getCmp("DirServiceDiagnosticRresultName" + id).getValue() + " (копия)");
...
function controllerDirServiceDiagnosticRresults_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirServiceDiagnosticRresultID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirServiceDiagnosticRresultID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceDiagnosticRresults + DirServiceDiagnosticRresultID + "/",
                    method: 'DELETE',
...
function controllerDirServiceDiagnosticRresults_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceDiagnosticRresults + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
