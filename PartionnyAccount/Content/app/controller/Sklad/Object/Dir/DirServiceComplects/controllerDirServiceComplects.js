Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirServiceComplects/controllerDirServiceComplects", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirServiceComplects': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirServiceComplects [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirServiceComplects [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirServiceComplects [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirServiceComplects [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirServiceComplects [itemId=tree]': {
            ...
            'viewDirServiceComplects button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirServiceComplects button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirServiceComplects button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirServiceComplects_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirServiceComplects_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirServiceComplects_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirServiceComplects + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirServiceComplects;
        if (parseInt(Ext.getCmp("DirServiceComplectID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirServiceComplects + "?id=" + parseInt(Ext.getCmp("DirServiceComplectID" + aButton.UO_id).value);
        }
...
function controllerDirServiceComplects_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirServiceComplects_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirServiceComplects_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirServiceComplectID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirServiceComplectID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceComplects + DirServiceComplectID + "/",
                    method: 'DELETE',
...
function controllerDirServiceComplects_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirServiceComplects + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
