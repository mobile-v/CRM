Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirMovementDescriptions/controllerDirMovementDescriptions", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirMovementDescriptions': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirMovementDescriptions [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirMovementDescriptions [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirMovementDescriptions [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirMovementDescriptions [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirMovementDescriptions [itemId=tree]': {
            ...
            'viewDirMovementDescriptions button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirMovementDescriptions button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirMovementDescriptions button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirMovementDescriptions_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirMovementDescriptions_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirMovementDescriptions_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirMovementDescriptions + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirMovementDescriptions;
        if (parseInt(Ext.getCmp("DirMovementDescriptionID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirMovementDescriptions + "?id=" + parseInt(Ext.getCmp("DirMovementDescriptionID" + aButton.UO_id).value);
        }
...
function controllerDirMovementDescriptions_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirMovementDescriptions_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirMovementDescriptions_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirMovementDescriptionID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirMovementDescriptionID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirMovementDescriptions + DirMovementDescriptionID + "/",
                    method: 'DELETE',
...
function controllerDirMovementDescriptions_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirMovementDescriptions + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
