Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirVats/controllerDirVats", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirVats': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirVats [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirVats [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirVats [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirVats [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirVats [itemId=tree]': {
            ...
            'viewDirVats button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirVats button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirVats button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirVats_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirVats_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirVats_onTree_folderDel(aButton.UO_id);
    },
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirVats;
        if (parseInt(Ext.getCmp("DirVatID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirVats + "?id=" + parseInt(Ext.getCmp("DirVatID" + aButton.UO_id).value);
        }
...
function controllerDirVats_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirVats_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirVats_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirVatID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirVatID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirVats + DirVatID + "/",
                    method: 'DELETE',
...
function controllerDirVats_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirVats + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
