Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirSmsTemplates/controllerDirSmsTemplates", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirSmsTemplates': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirSmsTemplates [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirSmsTemplates [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirSmsTemplates [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirSmsTemplates [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirSmsTemplates [itemId=tree]': {
            ...
            'viewDirSmsTemplates button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirSmsTemplates button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirSmsTemplates button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirSmsTemplates_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirSmsTemplates_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirSmsTemplates_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirSmsTemplates + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirSmsTemplates;
        if (parseInt(Ext.getCmp("DirSmsTemplateID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirSmsTemplates + "?id=" + parseInt(Ext.getCmp("DirSmsTemplateID" + aButton.UO_id).value);
        }
...
function controllerDirSmsTemplates_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);

    debugger;
    var UO_idTab = Ext.getCmp("viewDirSmsTemplates" + id).UO_idTab;
    Ext.getCmp("MenuID" + id).setValue(UO_idTab);

    Ext.getCmp("tree_" + id).disable();
};
function controllerDirSmsTemplates_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirSmsTemplates_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirSmsTemplateID" + id).setValue(0);
                Ext.getCmp("DirSmsTemplateName" + id).setValue(Ext.getCmp("DirSmsTemplateName" + id).getValue() + " (копия)");
...
function controllerDirSmsTemplates_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirSmsTemplateID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirSmsTemplateID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirSmsTemplates + DirSmsTemplateID + "/",
                    method: 'DELETE',
...
function controllerDirSmsTemplates_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirSmsTemplates + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
