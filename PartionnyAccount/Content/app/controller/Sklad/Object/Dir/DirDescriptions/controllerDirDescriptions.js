Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirDescriptions/controllerDirDescriptions", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirDescriptions': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirDescriptions [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirDescriptions [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirDescriptions [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirDescriptions [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirDescriptions [itemId=tree]': {
            ...
            'viewDirDescriptions button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirDescriptions button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirDescriptions button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirDescriptions_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirDescriptions_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirDescriptions_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirDescriptions + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirDescriptions;
        if (parseInt(Ext.getCmp("DirDescriptionID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirDescriptions + "?id=" + parseInt(Ext.getCmp("DirDescriptionID" + aButton.UO_id).value);
        }
...
function controllerDirDescriptions_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirDescriptions_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirDescriptions_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirDescriptionID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirDescriptionID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirDescriptions + DirDescriptionID + "/",
                    method: 'DELETE',
...
function controllerDirDescriptions_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirDescriptions + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
