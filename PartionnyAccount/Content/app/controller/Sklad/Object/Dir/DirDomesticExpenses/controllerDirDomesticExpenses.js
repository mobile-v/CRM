Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirDomesticExpenses/controllerDirDomesticExpenses", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirDomesticExpenses': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirDomesticExpenses [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirDomesticExpenses [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirDomesticExpenses [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirDomesticExpenses [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirDomesticExpenses [itemId=tree]': {
            ...
            'viewDirDomesticExpenses button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirDomesticExpenses button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirDomesticExpenses button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirDomesticExpenses_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirDomesticExpenses_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirDomesticExpenses_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirDomesticExpenses + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirDomesticExpenses;
        if (parseInt(Ext.getCmp("DirDomesticExpenseID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirDomesticExpenses + "?id=" + parseInt(Ext.getCmp("DirDomesticExpenseID" + aButton.UO_id).value);
        }
...
function controllerDirDomesticExpenses_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("Sign" + id).setValue(-1);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirDomesticExpenses_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("Sign" + id).setValue(-1);
...
function controllerDirDomesticExpenses_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirDomesticExpenseID" + id).setValue(0);
                Ext.getCmp("DirDomesticExpenseName" + id).setValue(Ext.getCmp("DirDomesticExpenseName" + id).getValue() + " (копия)");
...
function controllerDirDomesticExpenses_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirDomesticExpenseID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirDomesticExpenseID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirDomesticExpenses + DirDomesticExpenseID + "/",
                    method: 'DELETE',
...
function controllerDirDomesticExpenses_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirDomesticExpenses + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
