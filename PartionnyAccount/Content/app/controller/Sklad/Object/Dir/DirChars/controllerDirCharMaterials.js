Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharMaterials", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharMaterials': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharMaterials [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharMaterials [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharMaterials [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharMaterials [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharMaterials [itemId=tree]': {
            ...
            'viewDirCharMaterials button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharMaterials button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharMaterials button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharMaterials_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharMaterials_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharMaterials_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharMaterials + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharMaterials;
        if (parseInt(Ext.getCmp("DirCharMaterialID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharMaterials + "?id=" + parseInt(Ext.getCmp("DirCharMaterialID" + aButton.UO_id).value);
        }
...
function controllerDirCharMaterials_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharMaterials_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharMaterials_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharMaterialID" + id).setValue(0);
                Ext.getCmp("DirCharMaterialName" + id).setValue(Ext.getCmp("DirCharMaterialName" + id).getValue() + " (копия)");
...
function controllerDirCharMaterials_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharMaterialID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharMaterialID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharMaterials + DirCharMaterialID + "/",
                    method: 'DELETE',
...
function controllerDirCharMaterials_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharMaterials + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
