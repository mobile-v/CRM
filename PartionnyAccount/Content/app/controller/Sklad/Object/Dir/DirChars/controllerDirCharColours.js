Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharColours", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharColours': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharColours [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharColours [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharColours [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharColours [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharColours [itemId=tree]': {
            ...
            'viewDirCharColours button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharColours button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharColours button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharColours_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharColours_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharColours_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharColours + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharColours;
        if (parseInt(Ext.getCmp("DirCharColourID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharColours + "?id=" + parseInt(Ext.getCmp("DirCharColourID" + aButton.UO_id).value);
        }
...
function controllerDirCharColours_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharColours_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharColours_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharColourID" + id).setValue(0);
                Ext.getCmp("DirCharColourName" + id).setValue(Ext.getCmp("DirCharColourName" + id).getValue() + " (копия)");
...
function controllerDirCharColours_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharColourID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharColourID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharColours + DirCharColourID + "/",
                    method: 'DELETE',
...
function controllerDirCharColours_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharColours + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
