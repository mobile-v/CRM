Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharTextures", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharTextures': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharTextures [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharTextures [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharTextures [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharTextures [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharTextures [itemId=tree]': {
            ...
            'viewDirCharTextures button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharTextures button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharTextures button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharTextures_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharTextures_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharTextures_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharTextures + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharTextures;
        if (parseInt(Ext.getCmp("DirCharTextureID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharTextures + "?id=" + parseInt(Ext.getCmp("DirCharTextureID" + aButton.UO_id).value);
        }
...
function controllerDirCharTextures_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharTextures_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharTextures_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharTextureID" + id).setValue(0);
                Ext.getCmp("DirCharTextureName" + id).setValue(Ext.getCmp("DirCharTextureName" + id).getValue() + " (копия)");
...
function controllerDirCharTextures_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharTextureID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharTextureID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharTextures + DirCharTextureID + "/",
                    method: 'DELETE',
...
function controllerDirCharTextures_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharTextures + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
