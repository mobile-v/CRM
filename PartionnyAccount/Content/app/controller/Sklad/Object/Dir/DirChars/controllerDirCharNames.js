Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharNames", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharNames': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharNames [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharNames [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharNames [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharNames [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharNames [itemId=tree]': {
            ...
            'viewDirCharNames button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharNames button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharNames button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharNames_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharNames_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharNames_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharNames + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharNames;
        if (parseInt(Ext.getCmp("DirCharNameID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharNames + "?id=" + parseInt(Ext.getCmp("DirCharNameID" + aButton.UO_id).value);
        }
...
function controllerDirCharNames_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharNames_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharNames_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharNameID" + id).setValue(0);
                Ext.getCmp("DirCharNameName" + id).setValue(Ext.getCmp("DirCharNameName" + id).getValue() + " (копия)");
...
function controllerDirCharNames_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharNameID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharNameID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharNames + DirCharNameID + "/",
                    method: 'DELETE',
...
function controllerDirCharNames_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharNames + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
