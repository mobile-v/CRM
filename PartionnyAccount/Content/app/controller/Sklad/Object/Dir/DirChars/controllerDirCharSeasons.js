Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharSeasons", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharSeasons': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharSeasons [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharSeasons [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharSeasons [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharSeasons [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharSeasons [itemId=tree]': {
            ...
            'viewDirCharSeasons button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharSeasons button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharSeasons button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharSeasons_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharSeasons_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharSeasons_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        if (Ext.getCmp(view.grid.UO_idMain).UO_Function_Tree == undefined || !varSelectOneClick) {
            ...
            widgetXForm.load({
                ...
                waitMsg: lanLoading,
                url: HTTP_DirCharSeasons + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharSeasons;
        if (parseInt(Ext.getCmp("DirCharSeasonID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharSeasons + "?id=" + parseInt(Ext.getCmp("DirCharSeasonID" + aButton.UO_id).value);
        }
...
function controllerDirCharSeasons_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharSeasons_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharSeasons_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharSeasonID" + id).setValue(0);
                Ext.getCmp("DirCharSeasonName" + id).setValue(Ext.getCmp("DirCharSeasonName" + id).getValue() + " (копия)");
...
function controllerDirCharSeasons_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharSeasonID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharSeasonID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharSeasons + DirCharSeasonID + "/",
                    method: 'DELETE',
...
function controllerDirCharSeasons_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharSeasons + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
