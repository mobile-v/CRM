Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharStyles", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharStyles': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharStyles [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharStyles [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharStyles [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharStyles [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharStyles [itemId=tree]': {
            ...
            'viewDirCharStyles button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharStyles button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharStyles button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharStyles_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharStyles_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharStyles_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharStyles + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharStyles;
        if (parseInt(Ext.getCmp("DirCharStyleID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharStyles + "?id=" + parseInt(Ext.getCmp("DirCharStyleID" + aButton.UO_id).value);
        }
...
function controllerDirCharStyles_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharStyles_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharStyles_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharStyleID" + id).setValue(0);
                Ext.getCmp("DirCharStyleName" + id).setValue(Ext.getCmp("DirCharStyleName" + id).getValue() + " (копия)");
...
function controllerDirCharStyles_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharStyleID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharStyleID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharStyles + DirCharStyleID + "/",
                    method: 'DELETE',
...
function controllerDirCharStyles_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharStyles + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
