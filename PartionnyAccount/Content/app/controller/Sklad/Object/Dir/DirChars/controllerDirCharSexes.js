Ext.define("PartionnyAccount.controller.Sklad/Object/Dir/DirChars/controllerDirCharSexes", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDirCharSexes': { close: this.this_close },

            //Группа (itemId=tree)
            // Меню Группы
            'viewDirCharSexes [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDirCharSexes [itemId=FolderEdit]': { click: this.onTree_folderEdit },
            'viewDirCharSexes [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDirCharSexes [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDirCharSexes [itemId=tree]': {
            ...
            'viewDirCharSexes button#btnSave': { "click": this.onBtnSaveClick },
            'viewDirCharSexes button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDirCharSexes button#btnHelp': { "click": this.onBtnHelpClick },
        });
    },
    ...
    onTree_folderNew: function (aButton, aEvent) {
        controllerDirCharSexes_onTree_folderNew(aButton.UO_id);
    },
    onTree_FolderCopy: function (aButton, aEvent) {
        controllerDirCharSexes_onTree_folderCopy(aButton.UO_id);
    },
    onTree_folderDel: function (aButton, aEvent, aOptions) {
        controllerDirCharSexes_onTree_folderDel(aButton.UO_id);
    },
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        ...
        widgetXForm.load({
            ...
            waitMsg: lanLoading,
            url: HTTP_DirCharSexes + rec.get('id') + "/",
    ...
    onBtnSaveClick: function (aButton, aEvent, aOptions) {
        ...
        var sMethod = "POST";
        var sUrl = HTTP_DirCharSexes;
        if (parseInt(Ext.getCmp("DirCharSexID" + aButton.UO_id).value) > 0) {
            sMethod = "PUT";
            sUrl = HTTP_DirCharSexes + "?id=" + parseInt(Ext.getCmp("DirCharSexID" + aButton.UO_id).value);
        }
...
function controllerDirCharSexes_onTree_folderNew(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
    Ext.getCmp("tree_" + id).disable();
};
function controllerDirCharSexes_onTree_folderNewSub(id) {
    var widgetXForm = Ext.getCmp("form_" + id).reset(true);
...
function controllerDirCharSexes_onTree_folderCopy(id) {
    Ext.MessageBox.show({
        title: lanOrgName, msg: "Создать копию записи?", icon: Ext.MessageBox.QUESTION, buttons: Ext.Msg.YESNO, width: 300, closable: false,
        fn: function (buttons) {
            if (buttons == "yes") {
                Ext.getCmp("DirCharSexID" + id).setValue(0);
                Ext.getCmp("DirCharSexName" + id).setValue(Ext.getCmp("DirCharSexName" + id).getValue() + " (копия)");
...
function controllerDirCharSexes_onTree_folderDel(id) {
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                var DirCharSexID = Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.id; //Ext.getCmp("tree_" + id).view.getSelectionModel().getSelection()[0].data.DirCharSexID;
                //Запрос на удаление
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharSexes + DirCharSexID + "/",
                    method: 'DELETE',
...
function controllerDirCharSexes_onTree_folderSubNull(id) {
    return;
    ...
    Ext.MessageBox.show({
        ...
        fn: function (buttons) {
            if (buttons == "yes") {
                ...
                Ext.Ajax.request({
                    timeout: varTimeOutDefault,
                    url: HTTP_DirCharSexes + "?id=" + Ext.getCmp("tree_" + id).getSelectionModel().getSelection()[0].data.id + "&sub=0", // + overModel.data.id,
                    method: 'PUT',
                    success: function (result) {
                        var sData = Ext.decode(result.responseText);
                        if (sData.success == true) {
