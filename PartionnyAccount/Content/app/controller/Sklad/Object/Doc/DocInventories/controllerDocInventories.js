Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocInventories/controllerDocInventories", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocInventories': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocInventories [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocInventories button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocInventories button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocInventories button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocInventories button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocInventories button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocInventories button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocInventories #DateS': { select: this.onGrid_DateS },
            'viewDocInventories #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocInventories [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocInventories #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocInventories, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocInventoriesEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocInventoryID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocInventoryID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocInventories + DocInventoryID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocInventories);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocInventories); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocInventories
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocInventories); //, f.value, HTTP_DocInventories
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocInventories); //, textReal, HTTP_DocInventories
    },
