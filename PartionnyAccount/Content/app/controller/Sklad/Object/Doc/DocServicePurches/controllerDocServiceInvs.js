Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocServicePurches/controllerDocServiceInvs", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocServiceInvs': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocServiceInvs #SpisatS': {
            ...
            //Грид (itemId=grid)
            'viewDocServiceInvs button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocServiceInvs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocServiceInvs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocServiceInvs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocServiceInvs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocServiceInvs button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocServiceInvs #DateS': { select: this.onGrid_DateS },
            'viewDocServiceInvs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocServiceInvs [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocServiceInvs #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocServiceInvs, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocServiceInvsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocServiceInvID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocServiceInvID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocServiceInvs + DocServiceInvID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocServiceInvs);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocServiceInvs); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocServiceInvs
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocServiceInvs); //, f.value, HTTP_DocServiceInvs
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocServiceInvs); //, textReal, HTTP_DocServiceInvs
    },
