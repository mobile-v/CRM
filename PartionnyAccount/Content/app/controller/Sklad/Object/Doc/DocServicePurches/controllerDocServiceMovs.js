Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocServicePurches/controllerDocServiceMovs", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocServiceMovs': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocServiceMovs [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocServiceMovs button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocServiceMovs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocServiceMovs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocServiceMovs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocServiceMovs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocServiceMovs button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocServiceMovs #DateS': { select: this.onGrid_DateS },
            'viewDocServiceMovs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocServiceMovs [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocServiceMovs #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocServiceMovs, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocServiceMovsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocServiceMovID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocServiceMovID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocServiceMovs + DocServiceMovID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocServiceMovs);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocServiceMovs); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocServiceMovs
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocServiceMovs); //, f.value, HTTP_DocServiceMovs
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocServiceMovs); //, textReal, HTTP_DocServiceMovs
    },
