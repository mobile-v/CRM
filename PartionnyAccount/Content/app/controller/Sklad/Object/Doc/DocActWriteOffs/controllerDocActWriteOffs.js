Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocActWriteOffs/controllerDocActWriteOffs", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocActWriteOffs': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocActWriteOffs [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocActWriteOffs button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocActWriteOffs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocActWriteOffs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocActWriteOffs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocActWriteOffs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocActWriteOffs button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocActWriteOffs #DateS': { select: this.onGrid_DateS },
            'viewDocActWriteOffs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocActWriteOffs [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocActWriteOffs #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocActWriteOffs, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocActWriteOffsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocActWriteOffID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocActWriteOffID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocActWriteOffs + DocActWriteOffID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocActWriteOffs);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocActWriteOffs); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocActWriteOffs
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocActWriteOffs); //, f.value, HTTP_DocActWriteOffs
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocActWriteOffs); //, textReal, HTTP_DocActWriteOffs
    },
