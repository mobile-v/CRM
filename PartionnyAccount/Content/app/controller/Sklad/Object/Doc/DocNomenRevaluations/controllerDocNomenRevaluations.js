Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocNomenRevaluations/controllerDocNomenRevaluations", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocNomenRevaluations': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocNomenRevaluations [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocNomenRevaluations button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocNomenRevaluations button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocNomenRevaluations button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocNomenRevaluations button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocNomenRevaluations button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocNomenRevaluations button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocNomenRevaluations #DateS': { select: this.onGrid_DateS },
            'viewDocNomenRevaluations #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocNomenRevaluations [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocNomenRevaluations #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocNomenRevaluations, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocNomenRevaluationsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocNomenRevaluationID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocNomenRevaluationID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocNomenRevaluations + DocNomenRevaluationID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocNomenRevaluations);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocNomenRevaluations); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocNomenRevaluations
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocNomenRevaluations); //, f.value, HTTP_DocNomenRevaluations
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocNomenRevaluations); //, textReal, HTTP_DocNomenRevaluations
    },
