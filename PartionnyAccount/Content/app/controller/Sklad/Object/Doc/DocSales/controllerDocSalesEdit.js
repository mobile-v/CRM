Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocSales/controllerDocSalesEdit", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocSalesEdit': { close: this.this_close },

            //Сисок (itemId=tree)
            // Меню Списка
            'viewDocSalesEdit [itemId=expandAll]': { click: this.onTree_expandAll },
            'viewDocSalesEdit [itemId=collapseAll]': { click: this.onTree_collapseAll },
            'viewDocSalesEdit [itemId=FolderNew]': { click: this.onTree_folderNew },
            'viewDocSalesEdit [itemId=FolderNewSub]': { click: this.onTree_folderNewSub },
            'viewDocSalesEdit [itemId=FolderEdit]': { click: this.onTree_FolderEdit },
            'viewDocSalesEdit [itemId=FolderCopy]': { click: this.onTree_FolderCopy },
            'viewDocSalesEdit [itemId=FolderDel]': { click: this.onTree_folderDel },
            // Клик по Группе
            'viewDocSalesEdit [itemId=tree]': {
            ...
            //PanelParty
            // Клик по Гриду "Party"
            'viewDocSalesEdit [itemId=gridParty]': {
            ...
            'viewDocSalesEdit button#btnDirNomenReload': { click: this.onBtnDirNomenReloadClick },
            //Поиск
            'viewDocSalesEdit #TriggerSearchTree': {
            ...
            'viewDocSalesEdit [itemId=DirContractorID]': { select: this.onDirContractorIDSelect },
            'viewDocSalesEdit button#btnDirContractorEdit': { click: this.onBtnDirContractorEditClick },
            'viewDocSalesEdit button#btnDirContractorReload': { click: this.onBtnDirContractorReloadClick },
            //Склад - Перегрузить
            'viewDocSalesEdit [itemId=DirWarehouseID]': { select: this.onDirWarehouseIDSelect },
            'viewDocSalesEdit button#btnDirWarehouseEdit': { click: this.onBtnDirWarehouseEditClick },
            'viewDocSalesEdit button#btnDirWarehouseReload': { click: this.onBtnDirWarehouseReloadClick },

            //PanelDocumentAdditionally
            //Склад - Перегрузить
            'viewDocSalesEdit [itemId=DirVatValue]': { select: this.onDirVatValueSelect },
            'viewDocSalesEdit [itemId=Payment]': { change: this.onPaymentChange },

            //PanelDocumentDiscount
            'viewDocSalesEdit [itemId=Discount]': { change: this.onDiscount },
            'viewDocSalesEdit button#btnSpreadDiscount': { click: this.onBtnSpreadDiscount },
            'viewDocSalesEdit #SummOther': { select: this.onSummOther },
            'viewDocSalesEdit button#btnSpreadSummOther': { click: this.onBtnSpreadSummOther },

            //PanelDocumentPay
            'viewDocSalesEdit [itemId=PayZReport]': { change: this.onPayZReportChecked },

            //Грид (itemId=grid)
            'viewDocSalesEdit button#btnGridAddPosition': { click: this.onGrid_BtnGridAddPosition },
            //'viewDocSalesEdit menuitem#btnGridSelectionOfGoods': { click: this.onGrid_BtnGridSelectionOfGoods },
            'viewDocSalesEdit button#btnGridEdit': { click: this.onGrid_BtnGridEdit },
            'viewDocSalesEdit button#btnGridDelete': { click: this.onGrid_BtnGridDelete },
            'viewDocSalesEdit button#btnGridPayment': { click: this.onGrid_BtnGridPayment },

            
            // Клик по Гриду
            'viewDocSalesEdit [itemId=grid]': {
            ...
            'viewDocSalesEdit button#btnHeldCancel': { click: this.onBtnHeldCancelClick },
            'viewDocSalesEdit button#btnHelds': { click: this.onBtnHeldsClick },
            'viewDocSalesEdit menuitem#btnSave': { click: this.onBtnSaveClick },
            'viewDocSalesEdit menuitem#btnSaveClose': { click: this.onBtnSaveClick },
            'viewDocSalesEdit button#btnCancel': { "click": this.onBtnCancelClick },
            'viewDocSalesEdit button#btnHelp': { "click": this.onBtnHelpClick },
            //***
            'viewDocSalesEdit menuitem#btnPrintHtml': { click: this.onBtnPrintHtmlClick },
            'viewDocSalesEdit menuitem#btnPrintExcel': { click: this.onBtnPrintHtmlClick }
        });
    },
    ...
    onDirContractorIDSelect: function (combo, records) {
        ...
        if (records.data.DirDiscountID == null && parseFloat(records.data.DirContractorDiscount) >= 0) {
            ...
            var loadingMask = new Ext.LoadMask({ msg: 'Please wait...', target: Ext.getCmp("viewDocSalesEdit" + combo.UO_id) });
            loadingMask.show();
    ...
    onDirVatValueSelect: function (combo, records) {
        controllerDocSalesEdit_RecalculationSums(combo.UO_id, false);
    },
    //Скидка
    onPaymentChange: function (aTextfield, aText) {
        controllerDocSalesEdit_RecalculationSums(aTextfield.UO_id, false)
    },
        ...
        Ext.Msg.show({
            ...
            fn: function (btn) {
                if (btn == "yes") {
                    ...
                    funSpreadDiscount(aButton.UO_id, ArrPrice, controllerDocSalesEdit_RecalculationSums);
                }
            },
    ...
    onBtnSpreadSummOther: function (aButton, aEvent) {
        Ext.Msg.show({
            ...
            fn: function (btn) {
                if (btn == "yes") {
                    var ArrPrice = ["PriceCurrency", "PriceVAT", "SUMPriceCurrency", "DirCurrencyRate", "DirCurrencyMultiplicity", "Quantity"];
                    funSpreadSummOther(aButton.UO_id, ArrPrice, controllerDocSalesEdit_RecalculationSums);
                }
            },
            icon: Ext.window.MessageBox.QUESTION
        });
    ...
    onBtnHeldCancelClick: function (aButton, aEvent, aOptions) {
        Ext.MessageBox.show({
            ...
            fn: function (buttons) { if (buttons == "yes") { controllerDocSalesEdit_onBtnSaveClick(aButton, aEvent, aOptions); } }
        });
    },
    ...
    onBtnHeldsClick: function (aButton, aEvent, aOptions) {
        if (Ext.getCmp('HavePay' + aButton.UO_id).getValue() != 0) {
            Ext.MessageBox.show({
                ...
                fn: function (buttons) {
                    if (buttons == "yes") {
                        controllerDocSalesEdit_onBtnSaveClick(aButton, aEvent, aOptions);
                    }
                }
            });
        }
    ...
    onBtnPrintHtmlClick: function (aButton, aEvent, aOptions) {
        ...
        if (Ext.getCmp("DocSaleID" + aButton.UO_id).getValue() == null) { Ext.Msg.alert(lanOrgName, txtMsg066); return; }
...
//Функия сохранения
function controllerDocSalesEdit_onBtnSaveClick(aButton, aEvent, aOptions) {

    //Спецификация (табличная часть)
    var recordsDocSaleTab = [];
    var storeGrid = Ext.getCmp("grid_" + aButton.UO_id).store;
    storeGrid.data.each(function (rec) { recordsDocSaleTab.push(rec.data); });
    ...
    var sMethod = "POST";
    var sUrl = HTTP_DocSales + "?UO_Action=" + aButton.UO_Action;
    if (parseInt(Ext.getCmp("DocSaleID" + aButton.UO_id).value) > 0) {
        sMethod = "PUT";
        sUrl = HTTP_DocSales + "?id=" + parseInt(Ext.getCmp("DocSaleID" + aButton.UO_id).value) + "&UO_Action=" + aButton.UO_Action;
    }
    ...
    widgetXForm.submit({
        method: sMethod,
        url: sUrl,
        params: { recordsDocSaleTab: Ext.encode(recordsDocSaleTab) },
        ...
        success: function (form, action) {
            ...
            if (!Ext.getCmp('DocID' + aButton.UO_id).getValue()) {
                ...
                Ext.getCmp('DocSaleID' + aButton.UO_id).setValue(sData.DocSaleID);
                Ext.getCmp('NumberInt' + aButton.UO_id).setValue(sData.DocSaleID);
                Ext.Msg.alert(lanOrgName, lanDataSaved + "<br />" + txtMsg096 + sData.DocSaleID);

                Ext.getCmp('viewDocSalesEdit' + aButton.UO_id).setTitle(Ext.getCmp('viewDocSalesEdit' + aButton.UO_id).title + " (" + Ext.getCmp("DocSaleID" + aButton.UO_id).getValue() + ")");
...
function controllerDocSalesEdit_RecalculationSums(id, ShowMsg) { 

    fun_DocX_RecalculationSums(id, ShowMsg);
    return;

    //Стор для "Табличной части"
    var storeGrid = Ext.getCmp(Ext.getCmp("form_" + id).UO_idMain).storeGrid; //storeDocSaleTabsGrid; //можно так: Ext.getCmp("viewDocSalesEdit" + id).storeDocSaleTabsGrid;
...
function controllerDocSalesEdit_onTree_folderNew(id) {
    var Params = [
        "tree_" + id,
        true, //UO_Center
        true, //UO_Modal
...
function controllerDocSalesEdit_onTree_folderEdit(id) {
    var Params = [
        "tree_" + id,
        true, //UO_Center
        true, //UO_Modal
...
function controllerDocSalesEdit_onTree_folderNewSub(id) {
    var node = funReturnNode(id);
...
function controllerDocSalesEdit_onTree_folderCopy(id) {
    Ext.MessageBox.show({
...
function controllerDocSalesEdit_onTree_folderDel(id) {
...
function controllerDocSalesEdit_onTree_folderSubNull(id) {
    return;
...
function controllerDocSalesEdit_onTree_addSub(id) {
