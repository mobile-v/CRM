Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocMovements/controllerDocMovements", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocMovements': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocMovements [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocMovements button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocMovements button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocMovements button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocMovements button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocMovements button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocMovements button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocMovements #DateS': { select: this.onGrid_DateS },
            'viewDocMovements #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocMovements [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocMovements #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocMovements, rec.get('id'));
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocMovementID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocMovementID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocMovements + DocMovementID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocMovements);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocMovements); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocMovements
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocMovements); //, f.value, HTTP_DocMovements
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocMovements); //, textReal, HTTP_DocMovements
    },
