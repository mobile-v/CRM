Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocSecondHandPurches/controllerDocSecondHandInvs", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocSecondHandInvs': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocSecondHandInvs #SpisatS': {
            ...
            //Грид (itemId=grid)
            'viewDocSecondHandInvs button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocSecondHandInvs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocSecondHandInvs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocSecondHandInvs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocSecondHandInvs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocSecondHandInvs button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocSecondHandInvs #DateS': { select: this.onGrid_DateS },
            'viewDocSecondHandInvs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocSecondHandInvs [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocSecondHandInvs #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocSecondHandInvs, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocSecondHandInvsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocSecondHandInvID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocSecondHandInvID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocSecondHandInvs + DocSecondHandInvID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocSecondHandInvs);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocSecondHandInvs); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocSecondHandInvs
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocSecondHandInvs); //, f.value, HTTP_DocSecondHandInvs
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocSecondHandInvs); //, textReal, HTTP_DocSecondHandInvs
    },
