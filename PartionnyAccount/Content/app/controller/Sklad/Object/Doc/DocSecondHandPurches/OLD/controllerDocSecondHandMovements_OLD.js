Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocSecondHandPurches/controllerDocSecondHandMovements", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocSecondHandMovements': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocSecondHandMovements [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocSecondHandMovements button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocSecondHandMovements button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocSecondHandMovements button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocSecondHandMovements button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocSecondHandMovements button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocSecondHandMovements button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocSecondHandMovements #DateS': { select: this.onGrid_DateS },
            'viewDocSecondHandMovements #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocSecondHandMovements [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocSecondHandMovements #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocSecondHandMovements, rec.get('id'));
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocSecondHandMovementID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocSecondHandMovementID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocSecondHandMovements + DocSecondHandMovementID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocSecondHandMovements);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocSecondHandMovements); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocSecondHandMovements
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocSecondHandMovements); //, f.value, HTTP_DocSecondHandMovements
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocSecondHandMovements); //, textReal, HTTP_DocSecondHandMovements
    },
