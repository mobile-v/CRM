Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocSecondHandPurches/controllerDocSecondHandMovs", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocSecondHandMovs': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocSecondHandMovs [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocSecondHandMovs button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocSecondHandMovs button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocSecondHandMovs button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocSecondHandMovs button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocSecondHandMovs button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocSecondHandMovs button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocSecondHandMovs #DateS': { select: this.onGrid_DateS },
            'viewDocSecondHandMovs #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocSecondHandMovs [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocSecondHandMovs #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocSecondHandMovs, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocSecondHandMovsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocSecondHandMovID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocSecondHandMovID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocSecondHandMovs + DocSecondHandMovID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocSecondHandMovs);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocSecondHandMovs); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocSecondHandMovs
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocSecondHandMovs); //, f.value, HTTP_DocSecondHandMovs
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocSecondHandMovs); //, textReal, HTTP_DocSecondHandMovs
    },
