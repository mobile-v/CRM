Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocReturnVendors/controllerDocReturnVendors", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocReturnVendors': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocReturnVendors [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocReturnVendors button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocReturnVendors button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocReturnVendors button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocReturnVendors button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocReturnVendors button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocReturnVendors button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocReturnVendors #DateS': { select: this.onGrid_DateS },
            'viewDocReturnVendors #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocReturnVendors [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocReturnVendors #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocReturnVendors, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocReturnVendorsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocReturnVendorID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocReturnVendorID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocReturnVendors + DocReturnVendorID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocReturnVendors);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocReturnVendors); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocReturnVendors
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocReturnVendors); //, f.value, HTTP_DocReturnVendors
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocReturnVendors); //, textReal, HTTP_DocReturnVendors
    },
