Ext.define("PartionnyAccount.controller.Sklad/Object/Doc/DocAccounts/controllerDocAccounts", {
    //Расширить
    ...
    init: function () {
        this.control({
            ...
            //Закрыте
            'viewDocAccounts': { close: this.this_close },

            //Группа (itemId=tree)
            // Клик по Группе
            'viewDocAccounts [itemId=tree]': {
            ...
            //Грид (itemId=grid)
            'viewDocAccounts button#btnNew': { click: this.onGrid_BtnNew },
            'viewDocAccounts button#btnNewCopy': { click: this.Grid_BtnNewCopy },
            'viewDocAccounts button#btnEdit': { click: this.onGrid_BtnEdit },
            'viewDocAccounts button#btnDelete': { click: this.onGrid_BtnDelete },
            'viewDocAccounts button#btnHelp': { click: this.onGrid_BtnHelp },
            'viewDocAccounts button#btnImport': { click: this.onGrid_BtnImport },
            'viewDocAccounts #DateS': { select: this.onGrid_DateS },
            'viewDocAccounts #DatePo': { select: this.onGrid_DatePo },
            // Клик по Гриду
            'viewDocAccounts [itemId=grid]': {
                selectionchange: this.onGrid_selectionchange,
                itemclick: this.onGrid_itemclick,
                itemdblclick: this.onGrid_itemdblclick
            },
            'viewDocAccounts #TriggerSearchGrid': {
    ...
    onTree_itemclick: function (view, rec, item, index, eventObj) {
        funGridDoc(view.grid.UO_id, HTTP_DocAccounts, rec.get('id'));
    },
    ...
    onGrid_BtnNew: function (aButton, aEvent, aOptions) {
        ...
        ObjectEditConfig("viewDocAccountsEdit", Params);
    },
        ...
        Ext.MessageBox.show({
            ...
            fn: function (buttons) {
                if (buttons == "yes") {
                    ...
                    var DocAccountID = Ext.getCmp("grid_" + aButton.UO_id).view.getSelectionModel().getSelection()[0].data.DocAccountID;
                    //Запрос на удаление
                    Ext.Ajax.request({
                        timeout: varTimeOutDefault,
                        url: HTTP_DocAccounts + DocAccountID + "/",
                        method: 'DELETE',
    ...
    onGrid_DateS: function (dataField, newValue, oldValue) {
        funGridDoc(dataField.UO_id, HTTP_DocAccounts);
    },
    ...
    onTriggerSearchGridClick1: function (aButton, aEvent) {
        funGridDoc(aButton.UO_id, HTTP_DocAccounts); //, Ext.getCmp("TriggerSearchGrid" + aButton.UO_id).value, HTTP_DocAccounts
    },
    onTriggerSearchGridClick2: function (f, e) {
        if (e.getKey() == e.ENTER) {
            funGridDoc(f.UO_id, HTTP_DocAccounts); //, f.value, HTTP_DocAccounts
        }
    },
    onTriggerSearchGridClick3: function (e, textReal, textLast) {
        if (textReal.length > 2) funGridDoc(e.UO_id, HTTP_DocAccounts); //, textReal, HTTP_DocAccounts
    },
